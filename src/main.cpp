...
int main() {
    ...
    cl_mem aBuffer =
            clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), as.data(), &status);
    OCL_SAFE_CALL(status);
    OCL_SAFE_CALL(clEnqueueWriteBuffer(command_queue, aBuffer, CL_TRUE, 0, n * sizeof(float), as.data(), 0, nullptr, nullptr));

    cl_mem bBuffer =
            clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), bs.data(), &status);
    OCL_SAFE_CALL(status);
    OCL_SAFE_CALL(clEnqueueWriteBuffer(command_queue, bBuffer, CL_TRUE, 0, n * sizeof(float), bs.data(), 0, nullptr, nullptr));

    cl_mem output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, n * sizeof(float), nullptr, &status);
    OCL_SAFE_CALL(status);
    ...
    cl_program program = clCreateProgramWithSource(context, 1, &program_c_str, nullptr, &status);
    OCL_SAFE_CALL(status);
    ...
    size_t log_size = 0;
    status = clGetProgramBuildInfo(program, device, CL_PROGRAM_BUILD_LOG, 0, nullptr, &log_size);
    ...
    // Выставите все аргументы в кернеле через clSetKernelArg (as_gpu, bs_gpu, cs_gpu и число значений, убедитесь, что тип количества элементов такой же в кернеле)
    {
        ...
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), &output));
    ...
    // Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(command_queue, output, true, 0, sizeof(float) * n, cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
